...
function loadAll() {
  ...
  const enriched = services.map((svc) => {
    ...
    const reductions = commonReductions.reductions.map((r) => ({
      ...r,
...
function getAllReductionsFlat() {
  const { commonReductions } = loadAll();
  return commonReductions.reductions;
}

function findAdditionById(serviceId, additionId) {
  const svc = getService(serviceId);
  if (!svc) return null;
  return svc.additions.find((a) => a.id === additionId) || null;
}
