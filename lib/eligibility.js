...
function evaluate(serviceId, additionId, answers = {}) {
  ...
  // Short-circuit before checking for unanswered requirements: OR logic can already
  // be satisfied by one "met" requirement, and AND logic can already be defeated by
  // one "not_met" requirement, regardless of what else is still unanswered.
  let verdict;
  if (isOr && booleanResults.some((r) => r.status === "met")) {
    verdict = "likely_eligible";
  } else if (!isOr && booleanResults.some((r) => r.status === "not_met")) {
