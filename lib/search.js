...
// Item names carry descriptive suffixes like "(2024年度新設)" or "(Ⅰ)(Ⅱ)" that
// prevent a literal substring match against the user's query even when the
// query names the addition precisely. Compare against the name with trailing
// parenthetical annotations stripped instead.
...
function search(query, { serviceIds = null, limit = 5, includeReductions = true } = {}) {
  ...
  for (const svc of services) {
    for (const item of svc.additions) {
      ...
      score += nameMatchBonus(query, item.name);
      if (score > 0) {
